...
export default function BookmarksList() {
  ...
  // Logged-out state on white band
  if (!user) {
    return (
      <>
        <Hero
  ...
  // Logged-in: main content on white band
  return (
    <>
      <Hero
        title="My Bookmarks"
          ...
          {bookmarks.length === 0 ? (
            ...
            <ol className="grid gap-4 sm:gap-6 md:grid-cols-2 lg:grid-cols-4">
              {bookmarks.map((b) => {
                ...
                const isScheduled = opp?.id ? scheduledIds.includes(opp.id) : false;
